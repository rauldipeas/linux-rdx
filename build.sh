#!/bin/bash
set -e
sudo apt install libelf-dev #bison build-essential flex libncurses-dev libssl-dev
...
make CC=clang clean
make CC=clang -j `getconf _NPROCESSORS_ONLN`
make CC=clang deb-pkg LOCALVERSION=-rauldipeas